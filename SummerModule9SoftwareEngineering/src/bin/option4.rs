...
//   ordering, we just stop letting one class block the other.
//
...
//   2. Try to book the IO head (cheap, 10% CPU). If it fits, dispatch it.
//      → IO tasks make progress whenever a worker slot is free.
//   3. Try to book the CPU head (35% CPU). If it fits, dispatch it.
...
//
// Order matters a little here: we try IO before CPU. With the 70/30 mix
// that means IO drains first when both fit, which keeps worker slots full.
// Trying CPU first would risk blocking on CPU headroom when the cheap
// option was right there. Either order is defensible — the key is that
// neither queue can permanently starve the other, because the manager
// always tries both each pass.
...
fn run_manager(
    ...
    loop {
        // Termination check: generator is done AND both queues are empty.
        // We check this at the top of the loop so we don't accidentally
        // park on the condvar after the last task has been dispatched.
        ...
        // Try IO lane first. Note: peek + try_book + pop is not atomic, but
        // that's fine because the manager is the ONLY popper. Generators
        // only push. So if peek showed Some, pop will return Some — there's
        // no race that could empty the queue between our peek and our pop.
        ...
        // Try CPU lane. We try this even if we just dispatched an IO task —
        // there might still be CPU headroom AND a free slot for a CPU task.
        // (e.g. 0% used → dispatch 10% IO → still 90% headroom for a CPU task.)
        ...
        // If we couldn't make any progress this pass, sleep until something
        // changes: a worker finishes (frees CPU/slot), a generator pushes
        // a new task, or the queues get closed. All three call pool.notify().
        if !dispatched_anything {
            pool.wait_for_change();
        }
        // If we DID dispatch, loop back immediately and try again — there
        // may still be more work that fits. We only park when blocked.
    }
    drop(worker_tx); // tell workers to exit
}
