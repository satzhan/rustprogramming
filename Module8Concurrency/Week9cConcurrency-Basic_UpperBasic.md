...
fn share_data_between_threads_through_channel() {
    ...
    println!("Spawned a thread!");
    sender.send(5).unwrap(); // if message did not reach receiver, you get err
    std::thread::sleep(std::time::Duration::from_secs(3));
    println!("Main thread slept for 3 seconds");
...
fn share_data_between_threads_and_mutate() {
    ...
    // smart pointer which guarantees that only one thread with lock
...
fn rw_locks() {
    // comparing with mutex which does not separate reading and writing
